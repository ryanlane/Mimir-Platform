...
**Problem:** Three implementations exist (Python, Node.js, .NET) but only Python is wired into compose. The others have no clear status.

**Recommendation:** Delete `mimir-discovery/nodejs/` and `mimir-discovery/dotnet/`, or move them to a clearly-marked `experiments/` directory. Dead code in the project root creates confusion about what's canonical.

---

### 2. Nested Directory Structure in `mimir-api/`

**Problem:** `mimir-api/api-service/app/` has three levels of nesting before you reach real code. The `api-service/` wrapper appears to be an artifact of earlier project evolution.

**Recommendation:** Flatten to `mimir-api/app/` — remove the `api-service/` wrapper directory. This simplifies Dockerfile `COPY` paths, volume mount definitions, and general navigation.

---

### 3. Default Credentials Committed to Repo

**Problem:** `mimir-api/api-service/mimir-api.docker.env` hardcodes `mimir/mimir/mimir` for Postgres and other service credentials. These are used directly in compose.

**Recommendation:** Add a `.env.example` at the project root with placeholder values and reference it from compose via `env_file: .env`. Add a clear note in the README that defaults are for local dev only and should be changed before exposing the service on a network.
...
| Discovery service | Needs cleanup (dead alternatives) |
| Directory depth in `mimir-api/` | Worth flattening |
| Secrets / credentials | Functional but undocumented for prod |
