#!/usr/bin/env python3
"""
MQTT Connectivity Test for Mimir (Updated for aiomqtt v1.0)
...
async def test_mqtt_connectivity(broker_host="oak", broker_port=1883):
    ...
    try:
        ...
        async with Client(
            ...
            port=broker_port,
            identifier=device_id,  # Changed from client_id to identifier in v1.0
        ) as client:
            ...
            # Try to receive the message (simplified for v1.0)
            ...
            try:
                ...
                async for message in client.messages:
                    try:
                        ...
                        print(f"⚠️  Could not parse message: {parse_error}")
                        break
                    
                    # Check timeout
                    if asyncio.get_event_loop().time() - start_time > 5:
                        print(f"⚠️  Did not receive message within timeout")
                        break
                        
            except Exception as e:
                print(f"⚠️  Message reception test skipped: {e}")
...
async def test_mimir_presence_topics():
    ...
    try:
        async with Client(hostname=broker_host, identifier=f"{device_id}-test") as client:
            # Test status topic
...
async def main():
    """Run all MQTT tests"""
    print("🚀 Starting MQTT connectivity tests for Mimir (aiomqtt v1.0)\n")
    ...
    if basic_test:
        ...
        if mimir_test:
            ...
            print(f"\nNext steps:")
            print(f"1. Update your mimir-api config.py with correct MQTT broker settings ✅ (Already done: oak)")
            print(f"2. Install aiomqtt in the API service: pip install aiomqtt")
            print(f"3. Install mqtt_presence_client.py on your display devices")
            print(f"4. Restart the mimir-api service to enable MQTT presence")
        else:
