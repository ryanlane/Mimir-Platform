...
def _dev_lan_origin_regex() -> str | None:
    if not settings.debug:
        return None
    # Allow common local dev origins, private-network IPs, and mDNS hostnames
    # such as http://mimir.local:8080 during self-hosted LAN development.
    return r"^https?://((localhost|127\.0\.0\.1)(:\d+)?|([a-z0-9-]+\.local)(:\d+)?|(10\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|172\.(1[6-9]|2\d|3[01])\.\d+\.\d+)(:\d+)?)$"
