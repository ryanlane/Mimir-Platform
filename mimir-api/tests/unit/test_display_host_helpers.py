...
def test_mdns_host_accepted_without_server_side_resolution(monkeypatch):
    ...
    assert _host_resolves_to_private_network("oak.local") is True
    assert _normalize_public_host_hint("oak.local") == "oak.local"


def test_unresolvable_bare_hostname_rejected(monkeypatch):
    monkeypatch.setattr(socket, "getaddrinfo", _fail_getaddrinfo)
    assert _host_resolves_to_private_network("oak") is False
    assert _normalize_public_host_hint("oak") is None
